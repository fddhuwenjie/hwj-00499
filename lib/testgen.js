...
function generateJestTest(endpoint, spec) {
  ...
  if (hasRequiredParams || hasRequestBody) {
    lines.push('');
    if (hasRequiredParams) {
      const pathParams = requiredParams.filter(p => p.in === 'path');
      const queryParams = requiredParams.filter(p => p.in === 'query');
      if (pathParams.length > 0) {
        for (const p of pathParams) {
          const sample = generateSampleValue(p.schema, spec);
          lines.push(`    const ${p.name} = ${JSON.stringify(sample)}; // ${p.description || '必填参数'}`);
        }
      }
      if (queryParams.length > 0) {
        ...
        for (const p of queryParams) {
          const sample = generateSampleValue(p.schema, spec);
          lines.push(`      ${p.name}: ${JSON.stringify(sample)}, // ${p.description || '必填参数'}`);
        }
        lines.push(`    };`);
      }
    }
    if (hasRequestBody) {
      const bodySample = generateSampleValue(endpoint.requestBody.schema, spec);
      lines.push(`    const requestBody = ${JSON.stringify(bodySample, null, 2).split('\n').join('\n    ')};`);
    }
  }
  ...
  const respSchema = endpoint.responses[successStatus]?.schema;
  if (respSchema) {
    const { required } = getSchemaProperties(spec, respSchema);
  ...
  if (errorStatuses.length > 0) {
    for (const statusCode of errorStatuses.slice(0, 2)) {
